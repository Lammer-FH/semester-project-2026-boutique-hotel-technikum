...
> The quick-start script automatically creates `.env` files from the provided `.env-example` templates in both `frontend/` and `backend/` if they don't exist. **You NEED to set the environment variable `MYSQL_ROOT_PASSWORD` in backend/.env after creation**.
